...
session_start();
	...
	if (!isset($_SESSION["Admin"])) { //^ The Admin may access class pages through the Preview button in Edit Post
		//? Checks if the user accessing the page has purchased the class or not
		$sqlSubscription = "SELECT * FROM subscription WHERE UserID = '$UserID' AND ClassID = '$ClassID'";
		$checkSubscription = mysqli_query($con, $sqlSubscription);
		if (mysqli_num_rows($checkSubscription) <= 0 ) {
			echo '<script>alert("Please purchase this class for access!");
			window.location.href="../Payment.php?Classes='.$ClassID.'";</script>';
		}
	}
