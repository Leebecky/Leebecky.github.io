    ...
    if (isset($_POST["SaveBtn"]) || isset($_POST["PreviewBtn"])) {
      ...
      function ArrayThis($Input) {
       //? Removes the HTML tags from the string
        ...
        $RemoveFront = ltrim($RemoveTags, "*"); //? Removes the * generated at the very front of the string
        $ExplodeArray = explode("*", $RemoveFront); //? Converts the string into an array
            foreach ($ExplodeArray as $NextLevel) { //? Splits the array string further
                $MultiArray[] = explode("|", $NextLevel);
            }
        return json_encode($MultiArray, JSON_HEX_APOS); //? Returns the array as an encoded JSON
    }
