...
session_start();
    ...
    if (isset($_POST["CreateBtn"])) {
        ...
        if (stripos($NewUsername, "Admin") !== FALSE || stripos($NewUsername, "Login") !== FALSE) {
            //^ Reserves the word Admin for Administrators only
            echo '<script>alert("Your username is using a reserved keyword! Please remove the word Admin or Login from your username."); 
           ...
           do {
            ...
            alert("Please set a security password to be able to use the Forgot Password function");
             </script>';
        }
         mysqli_close($con);
    }
