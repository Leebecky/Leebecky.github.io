...
session_start();
	...
	if (isset($_SESSION["UserID"])) {
		...
		$sqlClass = "SELECT ClassID, Title, Type from classes WHERE ClassID = '$ClassID'";
...
<form action="Payment.php" method="POST">
					...
					<input type="hidden" name="Classes" value="<?php echo $ClassID; ?>">
						...
						<label for="PayMethodV">
							<input type="radio" id="PayMethodV" name="Paymethod">Visa 
...
<br><br>				<label class="PayLbl">Card Expiration Date and Security Code</label>
							<!-- ? minimum date allowed for expiry is the current month. -->
						<input type="month" id="CardExpiry" placeholder="MM/YY"  min="<?php echo date('Y-m'); ?>" required>
						<input type="number" id="CVV" placeholder="CVV" min="100" required>

						<!-- CVV Tooltip Hover -->
							<i class="fas fa-question-circle tooltip">
							<span class="tooltiptext">CVV is the three digits found on the back of your card</span></i>
<br><br>
...
<br>
					...
					<input type="text" id="PayTotal" value="RM <?php echo $RM; ?>" readonly></label>
<br><br>
				...
				onclick="PayConfirm(ValidateCard())">Pay</button>
			</div>
		</div>
