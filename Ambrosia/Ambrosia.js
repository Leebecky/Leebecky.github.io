...
function RadioRequired(Name, Action){ //^ Ensures that the radio buttons are selected at Add New Post (Class)
    var Radio = document.getElementsByName(Name);
    if (Action == "Set") {
            for (let index = 0; index < Radio.length; index++) {
                Radio[index].setAttribute("required", "required");
            }
    } else if (Action == "Remove") {
        for (let index = 0; index < Radio.length; index++) {
            if (Radio[index].hasAttribute("required")) {
                Radio[index].removeAttribute("required");
                console.log("need");
            }
        }
    }
...
function PostTypeDisplay() { //^ Changes the layout for posting according to the type of post
    ...
    var PostTagsAR = document.getElementById("TagsAR");

    if (Type == "Recipes") {
        Label.textContent = "Please ensure that the image is named as per the Title of the recipe!";
        ...
        PostTagsAR.setAttribute("required", "required");
        RadioRequired("ClassType", "Remove");
        RadioRequired("PostTags", "Remove");
        
    } else if (Type == "Articles") {
        Label.textContent = "Please ensure that the image is named as per the Title of the article!";
        ...
        PostTagsAR.removeAttribute("required");
        RadioRequired("ClassType", "Set");
        RadioRequired("PostTags", "Set");
    }
}
...
function Confirmation(Type, ID) { //^ to double check intentions to delete User/Admin/Post/Tag/Payment
  switch (Type) {
        case "Users":
...
function AdminSubmit() { //^ Collects data and submits the form to create a new Admin
   var AdminUsername = prompt("Username for Admins must contain the word Admin", "Admin");
    ...
    if (CheckTable(AdminUsername.toLowerCase()) == false) {
...
function ValidateCard() { //^ Validation for credit card numbers using Luhn's algorithm
    ...
    var validCC = 0,
        ...
        if (CardNum.length !== 13 && CardNum.length !== 16 && CardNum.length !== 19 ) { //? MasterCard/Visa credit card num length
            return alert("Sorry, your credit card number is invalid!");
        }

        while (i >= 0) {          
            num = parseInt(CardNum.charAt(i), 10);
            if (isNaN(num)) {
                return alert("Sorry, your credit card number is invalid!");
            }

            if (altNum) { //? Only runs when altNum = true (every 2nd digit from the right is evaluated as according to Luhn algorithm)
                console.log(num);
                num *=2;
                if (num > 9) {
                    num = (num - 10) + 1;
...
function SettingsSubmit(Status) {
    if (Status == "Confirm") {
        document.getElementById("UserSettings").submit();
    }
}
...
function PayConfirm(Status) {//^ Checks that all payment details have been filled before proceeding
    ...
    if (Status == "Confirm" && CardDetail !== "" && CardEx !== "" && CVV !== "" && CVV.length == 3
    && (RadioVisa.checked || RadioMasters.checked)) {
